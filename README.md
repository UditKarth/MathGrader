...
A single-page web app that turns raw **Desmos Math** Grade 1 assessment scores into a
**per-standard report** for each student — the view report cards and parent conferences
actually need.
...
- **Gradebook** — pick a unit and assessment, then type scores into a students × questions
  grid. Each question column carries the standards it's aligned to and an editable
  max-points field that applies to the whole class. Fully keyboard-driven.
...
Current output: **151 questions across 31 unit/assessment groups in 7 units, covering 32
distinct standards.**
...
- **Max points is a property of the question, not the student.** Setting "Q3 is worth 2
  points" applies to the whole class.
